...
The policy speaks two protocols. A constrained choice API answers the whole question set natively and returns a measured probability distribution over the offered ids; an OpenAI-compatible chat model answers every question in a single JSON reply, reporting one choice and one confidence per question, so the distribution shown in the inspector is spread from that confidence rather than measured per element. Both paths keep one request per decision cycle, and both validate every answer against the observed element ids before anything executes — a key the page never offered is rejected, not clamped.

Which path a run takes is decided by the endpoint host and then the model id. `typesafe.ai` is the choice API whatever model runs on it. OpenRouter serves both: a decisions model such as the shipped `typesafe/jev-1.13` is routed to `https://openrouter.ai/api/alpha/decisions` and returns measured probabilities, while any chat model on the same host goes to `/chat/completions`. A decisions model is rejected outright on the chat endpoint, so the routing keys on the model, not only the host. `TYPESAFE_ENDPOINT=https://api.typesafe.ai/v1/systemone` with `TYPESAFE_MODEL=jev-latest` reaches TypeSafe directly; the measurements below were taken on that path.
